...
class ReportGenerator:
    ...
    def analyze_transcript(self, video_info: Dict[str, Any], transcript: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            - CRITICAL: Do not include any preliminary text before the JSON begins or after it ends
            - CRITICAL: All string values must be properly escaped within the JSON
            - CRITICAL: Return ONLY the JSON object, nothing else before or after
            - CRITICAL: Make sure the JSON has no trailing commas and is fully valid
            - CRITICAL: Use double quotes for all keys and string values
            - CRITICAL: Test that your response is valid JSON before submitting
            - If you need to include quotes inside string values, escape them with a backslash

            TRANSCRIPT:
            {transcript[:90000]}  # Truncate if too long
            """

            # Call Anthropic API using the appropriate method based on version
            response_text = ""
            try:
                print(f"Calling Anthropic API ({self.api_version}) for video: {video_info['id']}")
            ...
            try:
                ...
                if start_idx >= 0 and end_idx > start_idx:
                    ...
                    print(f"Found JSON string from index {start_idx} to {end_idx}")
                    try:
                        analysis = json.loads(json_str)
                        print("Successfully parsed JSON")
                    except json.JSONDecodeError as json_err:
                        print(f"JSON parsing error: {json_err}")
                        print(f"JSON string excerpt: {json_str[:100]}...{json_str[-100:] if len(json_str) > 100 else ''}")
                    ...
                    print(f"Could not find JSON in response. Start index: {start_idx}, End index: {end_idx}")
                    # Extract summary from the text if possible
                    summary = response_text.strip()[:1000]
                ...
                print(f"Error processing response: {json_error}")
                # Structured format if JSON parsing fails
                analysis = {
                    "main_topics": ["Unable to parse main topics"],
                    "key_points": ["Unable to parse key points"],
                    ...
                    "target_audience": "General audience",
                    "content_quality": "Unable to assess",
                    "overall_summary": response_text[:1000],
                    "detailed_summary": response_text[:2000]
                }
